...
# Editor shows the current color in tk's '#rrggbb' hexadecimal format
#
# (c) 2013 PLTW
# 
# 
...
#######
class ColorSlider(tk.Scale): # ColorSlider is a subclass of tk.Scale
    ...
    def __init__(self, parent, myLabel, model_intvar, editor, canvas):
        ...
        # tk.Scale, associated with the model data and the event handler
...
for x in range(10,300,40):
    ...
    new_circle = canvas.create_rectangle(x-r, y-r, x+r, y+r, outline='#000000')
...
listofthing=["1","2","3","4","5","6","7","8","9","A","B","C","D","E","F"]

def randomthing(event):
    print("rando pressed")
    canvas.create_oval(140, 75, 190, 95,
                                    fill='#FFFF00', outline='#000000')

# Subscribe handlers to the Button-1 and ButtonRelease-1 events
canvas.bind('<Button-1>', down)
canvas.bind('<ButtonRelease-1>', up)
canvas.bind('<r>', randomthing)
canvas.focus_set()

######
# Functions to transform Intvars into tk color strings
#######
...
def color(r,g,b):
    '''Takes three IntVar and returns a color tk string like #FFFFFF.        
    '''
    rx=hexstring(r)
    gx=hexstring(g)
    bx=hexstring(b)
